package com.chilun.apiopenspace.gateway.filter.check;
...
public class RequestCheckFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        try {
            ...
            return ResponseUtils.ErrorResponse("校验异常：请求头参数为空或格式错误", HttpStatus.BAD_REQUEST, exchange);
        }
        ...
        if (expireTimestamp < System.currentTimeMillis() || System.currentTimeMillis() < sendTimestamp || salt > 2000 || salt < 100) {
            ...
            return ResponseUtils.ErrorResponse("校验异常：请求头参数数值异常", HttpStatus.BAD_REQUEST, exchange);
        }
        ...
        if (oldTimestamp != null && sendTimestamp <= oldTimestamp) {
            ...
            return ResponseUtils.ErrorResponse("校验异常：请求重复或过期，旧时间戳：" + oldTimestamp, HttpStatus.BAD_REQUEST, exchange);
        }
        ...
        if (interfaceAccessInRedis != null) {
            ...
            //四、设置标注，说明该请求已被验证通过，后续如果发生异常，需进入日志记录
            ...
            return accessService.getCryptographicInterfaceAccess(accesskey).flatMap(stringBaseResponse -> {
                ...
                try {
                    ...
                    return ResponseUtils.ErrorResponse("校验异常：accesskey无效", HttpStatus.BAD_REQUEST, exchange);
                }
                try {
                    ...
                    return ResponseUtils.ErrorResponse("校验异常：校验过程出现异常", HttpStatus.INTERNAL_SERVER_ERROR, exchange);
                }
                //4校验签名
                ...
                //五、设置标注，说明该请求已被验证通过，后续如果发生异常，需进入日志记录
                exchange.getAttributes().put(ExchangeAttributes.NEED_IN_LOG, "true");
                return chain.filter(exchange);
            });
        }
    ...
    private ResponseUtils.ResponseEntity verifyRequest(long sendTimestamp, String accesskey, int salt, String sign, String uri, InterfaceAccess interfaceAccess) {
        ...
        if (interfaceAccess == null || interfaceAccess.getInterfaceId() == null) {
            ...
            return new ResponseUtils.ResponseEntity("校验异常：校验过程出现未知异常，请联系管理员", HttpStatus.INTERNAL_SERVER_ERROR);
        } else if (routeId != interfaceAccess.getInterfaceId()) {
            log.info("访问码与接口不对应，访问码可用接口：{}，实际访问接口：{}。", interfaceAccess.getInterfaceId(), routeId);
            return new ResponseUtils.ResponseEntity("校验异常：访问码与接口不对应", HttpStatus.BAD_REQUEST);
        }
        ...
        if (interfaceAccess.getVerifyType() == null) {
            ...
            try {
                if (!SignatureUtils.verifySignature(accesskey, secretkey, uri, sendTimestamp, salt, sign)) {
                    ...
                    return new ResponseUtils.ResponseEntity("校验异常：签名错误", HttpStatus.BAD_REQUEST);
                }
